      ...
      library
        ...
        .then((abi) => {
          ...
          Promise.all([
            ...
            .then(([{ token }, owner, tokenURI, status, assetId, openable, collectable]) => {
              library
      ...
      library
        ...
        .then((abi) => {
          ...
          Promise.all([
            ...
            Promise.all(
              token.collectable.map((tokenId) =>
                Promise.all([Promise.resolve(tokenId), library.contractCall(contract, 'viewERC20s', [tokenId])])
              )
            ),
          ])
            .then(([avax, coins]) => {
